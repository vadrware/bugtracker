...
urlpatterns = patterns('',
    (r'^/$', login_required(object_list), 
        dict(queryset = ProductVersion.objects.all(), paginate_by = 20)),
    (r'^/detail/(?P<object_id>\d+)/?$', login_required(object_detail), 
        dict(queryset = ProductVersion.objects.all())),
    (r'^/create/?$', 'django.views.generic.create_update.create_object', 
        dict(model = ProductVersion, login_required = True)),
    ...
    (r'^/delete/(?P<object_id>\d+)/?$', 'django.views.generic.create_update.delete_object', 
        dict(model = ProductVersion, login_required = True, post_delete_redirect = "/productversions")),
)
